...
class Login:
    ...
    def download_trackdee(self, URL, output=localdir + "/Songs/", check=True, quality="MP3_320", recursive=True):
        ...
        try:
            ...
            try:
                ...
                try:
                    url = self.request(
                        "https://api.deezer.com/search/track/?q=" + music[0].replace("#", "").split(" ")[0] + " + " +
                        artist[0].replace("#", "")).json()
                    try:
                        if url['error']['message'] == "Quota limit exceeded":
                            raise QuotaExceeded("Too much requests limit yourself")
                    except KeyError:
    ...
    def download_albumdee(self, URL, output=localdir + "/Songs/", check=True, quality="MP3_320", recursive=True):
        ...
        for a in tqdm(range(len(urls))):
            ...
            try:
                ...
                url = self.request(
                    "https://api.deezer.com/search/track/?q=" + music[a].replace("#", "") + " + " + artist[a].replace(
                        "#", "")).json()
                try:
                    if url['error']['message'] == "Quota limit exceeded":
                        raise QuotaExceeded("Too much requests limit yourself")
                ...
                try:
                    ...
                    try:
                        url = self.request(
                            "https://api.deezer.com/search/track/?q=" + music[a].replace("#", "").split(" ")[
                                0] + " + " + artist[a].replace("#", "")).json()
                        try:
                            if url['error']['message'] == "Quota limit exceeded":
                                raise QuotaExceeded("Too much requests limit yourself")
                        except KeyError:
    ...
    def download_playlistspo(self, URL, output=localdir + "/Songs/", check=True, quality="MP3_320", recursive=True):
        ...
        for a in tracks['items']:
            try:
                array.append(
                    self.download_trackspo(a['track']['external_urls']['spotify'], output, check, quality, recursive))
            except TrackNotFound:
        ...
        if tracks['total'] != 100:
            for a in range(tracks['total'] // 100):
                ...
                for a in tracks['items']:
                    try:
                        array.append(
                            self.download_trackspo(a['track']['external_urls']['spotify'], output, check, quality,
                                                   recursive))
                    except:
                        print("\nTrack not found " + a['track']['name'])
                        array.append(output + a['track']['name'] + "/" + a['track']['name'])
